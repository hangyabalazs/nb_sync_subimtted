...
addParameter(prs,'sessType','behavior',@(s)ischar(s)&ismember(s,{'behavior',...
    'burstOn', 'stimFA', 'stimHIT', 'CR', 'Miss', 'CueHit', 'CueFA', 'CRhit'...
    'Missfa', 'fb_exclude', 'fb_exclude_new'}))   % session type filter
    ...
    for bi = 1:length(burstfilter) % Burstfilter loop
        ...
        for iS = 1:length(sync) % Sync loop
            ...
            if ~strcmp('original', sMode) % Sync, NonSync loop
                ...
                load([RESDIR cb fs 'ccg' cb fs 'behavior' fs burstfilter cb fs...
                    'CCG_matrices_' 'behavior' '_' burstfilter '_' cb '.mat']);
    ...
    switch sessType
        ...
        case 'CueHit'
            ...
            try
                ...
                unit = TE.TrialStart(hit_trials) + TE.(alignevent_hit)(hit_trials);
            catch
                disp([cellid ': There was no behavioral protocol for this session.'])
                unit = [];
            end
        case 'CueFA'
            alignevent_fa = 'StimulusOn';
            ...
            try
                ...
                unit = TE.TrialStart(fa_trials) + TE.(alignevent_fa)(fa_trials);
            catch
                disp([cellid ': There was no behavioral protocol for this session.'])
                unit = [];
            end
        case 'CRhit'
            ...
            try
                ...
                % Segments to include
                hit_trials = ~isnan(TE.CorrectRejection);  % correct rejection trials
                unit = TE.TrialStart(hit_trials) + TE.(alignevent_hit)(hit_trials);
            catch
                disp([cellid ': There was no behavioral protocol for this session.'])
                unit = [];
        ...
        case 'Missfa'
            ...
            try
                ...
                % Segments to include
                fa_trials = ~isnan(TE.Miss);  % false alarm trials
                unit = TE.TrialStart(fa_trials) + TE.(alignevent_fa)(fa_trials);
            catch
                disp([cellid ': There was no behavioral protocol for this session.'])
                unit = [];
...
ff1 = [resdir '\STAplots\' regexprep(sessType,' ','_') '\' sMode PATH '\'...
    group '_' cellidt '_' cellidt2 '_' num2str(index) '_' 'STA_new_' numVer '.fig'];   % save STA figure
ff2 = [resdir '\STAjpegs\'  regexprep(sessType,' ','_') '\'  sMode PATH '\'...
    group '_' cellidt '_' cellidt2 '_' num2str(index) '_' 'STAimage_new_' numVer '.jpg'];   % save STA jpeg
saveas(H2,ff2)
saveas(H2,ff1)
close(H2);

% Drop spikes which is at the very beginning of the recording
% out of the window (wn) range
vdisc = vdisc((vdisc-wn/2)>=0);
...
ff1 = [resdir '\STAspect\' regexprep(sessType,' ','_') '\' sMode PATH '\' group '_' cellidt...
    '_' cellidt2 '_' num2str(index) '_' 'STAspect_new_' numVer '.fig'];   % save spectrum
ff2 = [resdir '\STAjpegs\' regexprep(sessType,' ','_') '\' sMode PATH '\' group '_' cellidt...
    '_' cellidt2 '_' num2str(index) '_' 'STAspect_new_' numVer '.jpg'];   % save spectrum jpg
saveas(H3,ff1)
saveas(H3,ff2)
close(H3);
...
ff3 = [resdir '\STAspect\' regexprep(sessType,' ','_') '\' sMode PATH '\' group '_' cellidt...
    '_' cellidt2 '_' num2str(index) '_' 'STAphase_new_' numVer '.fig'];   % save spectrum
ff4 = [resdir '\STAjpegs\' regexprep(sessType,' ','_') '\' sMode PATH '\' group '_' cellidt...
    '_' cellidt2 '_' num2str(index) '_' 'STAphase_new_' numVer '.jpg'];   % save spectrum jpg
saveas(H4,ff3)
saveas(H4,ff4)
close(H4);

staFile = [resdir '\SpectData\' regexprep(sessType,' ','_') '\' sMode PATH...
    '\' cellidt 'staMatrix_new_' numVer '.mat'];
phaseFile = [resdir '\SpectData\' regexprep(sessType,' ','_') '\' sMode PATH...
    '\' cellidt 'phaseMatrix_new_' numVer '.mat'];
spectFile = [resdir '\SpectData\' regexprep(sessType,' ','_') '\' sMode PATH...
    '\' cellidt 'spectMatrix_new_' numVer '.mat'];
