...
if ~isempty(spt)
    ...
    if minafter > baseline_prob     % putative inhibition, if it goes below baseline
        ...
        for k = 1:wnnm
            ...
            if isempty(mcw)
                %             error('There was no sync events between the cells')
            end
            mcw = mcw(1);
    ...
    % Output statistics set to NaN;
    stats.baseline = NaN;
    stats.minvalue = NaN;
    stats.inhibition_start = NaN;
    stats.inhibition_end = NaN;
    stats.inhibition_peak = NaN;
    stats.inhibition_time = NaN;
    stats.Wpi = NaN;
    stats.maxvalue = NaN;
    stats.activation_start = NaN;
    stats.activation_end = NaN;
    stats.activation_peak = NaN;
    stats.activation_time = NaN;
    stats.Wpa = NaN;
end