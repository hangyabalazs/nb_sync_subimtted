...
%     	'burstOn' - tagging segments during stimulation and behavior
%           protocols
...
% -------------------------------------------------------------------------
function [tseg g] = burstOn(g,cellid)
...
% -------------------------------------------------------------------------
function [tseg g] = burstOff(g,cellid)
...
try
    ...
    % During stimulation protocols: from pulse onset, using light activation duration argument
    stim_segs = [SE.PulseOn + g.light_activation_duration(1); ...
        SE.PulseOn + g.light_activation_duration(2)];
catch
    disp([cellid ': There was no stim protocol for this session.'])
...
% -------------------------------------------------------------------------
function [tseg g] = ITI(g,cellid)

% align to ITI onset
alignevent = 'LastITIBegins';
wn = 0.25; % 250ms margin
...
try
    % Load trial events
    TE = loadcb(cellid,'TrialEvents');
    startTime = TE.TrialStart;
    % Segments to include
    tseg = [TE.LastITIBegins+startTime; TE.StimulusOn+startTime];
catch
    disp([cellid ': There was no behavioral protocol for this session.'])
    tseg = [];
end